...
```{r}
dc = read.csv('data/population-by-country-2015.csv')
...
Regions with more than 100 confirmed cases increased rapidly from a few to many.
...
```{r}
ggplot(subset(m5, Region=='US' | Region=='France' | Region=='Italy' | Region =='Canada' |
                Region=='Switzerland' | Region=='Iran' | Region=='China' | Region=='Spain' | Region == 'Korea, South'), aes(Date, Confirmed, col=Region)) + 
  geom_point() + 
...
for(q in 1:nrow(q.dates)) {
  ...
  if (!(q.dates$Region[q] %in% c('Cruise Ship','Denmark','Qatar','Diamond Princess','Serbia'))) {
...
ggplot(m8, aes(DateDaysNorm, Confirmed/Population*1e6, col=Region)) + 
  ...
  scale_x_continuous(limits=c(0,35)) + xlab('Days since 100th confirmed case') + 
  geom_label(data=subset(m8z, Population>7e6),aes(label=Region),hjust=-0.2) + 
  theme_bw() + theme(legend.position = 'none')
```

```{r}
m8s = subset(m8, Region=='US' | Region=='Spain' | Region=='Korea, South' | Region =='Sweden' |
               Region=='Switzerland' | Region=='Japan' | Region=='China' | Region=='Italy')
m8s %>% 
  group_by(Region) %>%
  filter(DateDaysNorm<40) %>%
  summarize(Confirmed = max(Confirmed),
            Population=max(Population), 
            DateDaysNorm = max(DateDaysNorm)) -> m8z
ggplot(m8, aes(DateDaysNorm, Confirmed/Population*1e6, col=Region)) + 
  ...
  scale_x_continuous(limits=c(0,44)) + xlab('Days since 100th confirmed case') + 
  geom_label(data=m8z,aes(label=Region),hjust=-0.2) + 
...
nls(data = m8a,
    ...
    start = list( T1=3)) -> fit
m8b = data.frame(DateDaysNorm = 0:30,
                 Confirmed = predict(fit, list(DateDaysNorm=0:30)))
...
ggplot(m8a, aes(DateDaysNorm, Confirmed)) + 
  ...
  scale_x_continuous(limits=c(0,28), breaks=0:4*7, minor_breaks=0:30) +
  theme_bw()
summary(fit)
```
...
ggplot(m8a, aes(DateDaysNorm, Confirmed)) + 
  ...
  ggtitle(paste('US since',min(m8a$Date),' doubling time:',T1,'days')) + xlab('days') +
  scale_y_log10nice(name='Confirmed COVID-19 cases') + 
  scale_x_continuous(limits=c(0,35), breaks=0:5*7, minor_breaks=0:30) +
...
Growth rates since 100th confirmed case in different countries for the first 10 days compared with the range from 10-25 days. If the dark line is on the right, then the doubling time is decreasing; if the dark line is left, it means that doubling time is getting faster.

```{r}
result = data.frame()
for (r in levels(droplevels(m8$Region))) {
  m8a = subset(m8, Region == r)
  m8b = subset(m8a, DateDaysNorm <= 10)
  if (nrow(m8b)>5) {
    A0 = m8b$Confirmed[1]
    nls(data = m8b,
      Confirmed ~ A0*exp(DateDaysNorm/T1),
      start = list(T1=3)) -> fit
    m8b = subset(m8a, DateDaysNorm >= 10 & DateDaysNorm <= 25)
...
ggplot(result, aes(x=reorder(Region,-T2), y=T2)) + 
  ...
  scale_y_log10(limits=c(2,30)) + ylab('confirmed cases doubling time (days)') + 
  coord_flip() +
  theme_bw() +
  theme(legend.position = 'none')
```
...
ggplot(subset(m5, Region=='US' | Region=='France' | Region=='Italy' | Region=='China' |
  ...
  geom_point() + 
  scale_x_continuous(limits=c(-30,50)) + xlab('days since Valentine')+ 
  scale_y_log10nice(limits=c(2,2e4), name='COVID-19 Deaths') + ylab('Deaths') + 
  scale_shape_manual(values=1:nlevels(m5$Region)) +
  theme_bw()
```


Renormalizing the start dates for `recorded deaths`, we find the dates when the `10th death` was recorded in each country:

```{r}
m6= subset(m5, Confirmed >= 10)
...
ggplot(subset(m7, Region %in% c.list), aes(DateDaysNorm, Confirmed, col=Region)) + 
  geom_path(data=m7, col='grey') + 
  geom_point() + geom_path() +
  scale_x_continuous(limits=c(-0,30)) + xlab('days since 15th death in region')+ 
  scale_y_log10nice(limits=c(10,1e4), name='COVID-19 Deaths') + ylab('Deaths') + 
  scale_shape_manual(values=1:nlevels(m5$Region)) +
  theme_bw()
...
m7u = subset(m7v, DateDaysNorm >= 10)
nls(data = m7u, Confirmed ~ A*exp((DateDaysNorm-10)/T),
    start = list(A=min(m7u$Confirmed), T =3)) -> fit
m7u.fit = data.frame(DateDaysNorm = 0:30,
                     Confirmed = predict(fit, list(DateDaysNorm=0:30)))
m7u.fit$Region = 'US'
ggplot(m7v, aes(DateDaysNorm, Confirmed, col=Region)) + 
  ...
  xlab('days since 15th death in region')+ 
  scale_y_log10nice(limits=c(12,2e4), name='COVID-19 Deaths') +  
  scale_shape_manual(values=1:nlevels(m5$Region)) +
  theme_bw() + theme(legend.position = c(0.01,0.95),
