...
m8b = data.frame(DateDaysNorm = 40:80,
                 Confirmed = predict(fit, list(DateDaysNorm=40:80)))
...
ggplot(m8a, aes(DateDaysNorm, Confirmed)) + 
  ...
  scale_x_continuous(limits=c(0,70), breaks=0:8*7, minor_breaks=0:80) +
  theme_bw()
summary(fit)
```
...
ggplot(m8a, aes(DateDaysNorm, Confirmed)) + 
  ...
  ggtitle(paste('US since',min(m8a$Date),' doubling time:',T1,'days')) + xlab('days') +
  scale_y_log10nice(name='Confirmed COVID-19 cases') + 
  scale_x_continuous(limits=c(0,74), breaks=0:12*7, minor_breaks=0:78) +
...
m8 %>%
  ...
  geom_path(size=1) +
  scale_x_continuous(limits=c(-0,80)) + xlab('days since 50th death in region')+ 
