...
m5 %>%
  ...
  scale_y_log10nice(limits=c(1000,3e6),name='confirmed COVID-19 cases') + 
  theme_bw()
...
ggplot(m8, aes(DateDaysNorm, Confirmed, col=Region)) + 
  ...
  scale_x_continuous(limits=c(0,70)) + xlab('Days since 100th confirmed case') + 
  theme_bw() + 
...
m8 %>% 
  group_by(Region) %>%
  filter(DateDaysNorm<70) %>%
  filter(Population>25e6) %>%
...
ggplot(m8z, aes(DateDaysNorm, Confirmed/Population*1e6, col=Region)) + 
  geom_line(data=subset(m8, Population>25e6), size=1, alpha=0.8) + 
  geom_line(data=subset(m8, Population<25e6), alpha=0.3) + 
  scale_y_log10nice(limits=c(60,2e4), name='Confirmed COVID-19 Cases per Million Population') + 
  scale_x_continuous(limits=c(0,80)) + xlab('Days since 1000th confirmed case') + 
...
ggplot(m8, aes(DateDaysNorm, Confirmed/Population*1e6, col=Region)) + 
  ...
  scale_x_continuous(limits=c(0,80)) + xlab('Days since 100th confirmed case') + 
...
A0 = m8a$Confirmed[1]
nls(data = subset(m8a, DateDaysNorm>55),
    Confirmed ~ A*exp(DateDaysNorm/T1),
    start = list( A=m8a$Confirmed[10], T1=4)) -> fit
m8b = data.frame(DateDaysNorm = 40:90,
                 Confirmed = predict(fit, list(DateDaysNorm=40:90)))
...
ggplot(m8a, aes(DateDaysNorm, Confirmed)) + 
  ...
  scale_x_continuous(limits=c(0,90), breaks=0:15*7, minor_breaks=0:110) +
  theme_bw()
summary(fit)
```
...
ggplot(m8a, aes(DateDaysNorm, Confirmed)) + 
  ...
  ggtitle(paste('US since',min(m8a$Date),' doubling time:',T1,'days')) + xlab('days') +
  scale_y_log10nice(name='Confirmed COVID-19 cases') + 
  scale_x_continuous(limits=c(0,100), breaks=0:18*7, minor_breaks=0:105) +
...
m5 %>%
  ...
  geom_point() + 
  scale_x_continuous(limits=c(-30,120)) + xlab('days since Valentine')+ 
  scale_y_log10nice(limits=c(500,2e5), name='COVID-19 Deaths') + ylab('Deaths') + 
...
ggplot(subset(m7, Region %in% c.list), aes(DateDaysNorm, Confirmed, col=Region)) + 
  ...
  geom_point() + 
  scale_x_continuous(limits=c(-0,100)) + xlab('days since 50th death in region')+ 
  scale_y_log10nice(limits=c(50,2e5), name='COVID-19 Deaths') + ylab('Deaths') + 
  scale_shape_manual(values=1:nlevels(m5$Region)) +
  theme_bw()
```
...
m7u = subset(m7v, DateDaysNorm >= 50)
nls(data = m7u, Confirmed ~ A*exp((DateDaysNorm-30)/T),
    start = list(A=min(m7u$Confirmed), T =3)) -> fit
m7u.fit = data.frame(DateDaysNorm = 0:110,
                     Confirmed = predict(fit, list(DateDaysNorm=0:110)))
m7u.fit$Region = 'US'
ggplot(m7v, aes(DateDaysNorm, Confirmed, col=Region)) + 
  ...
  scale_y_log10nice(limits=c(110,2e5), name='COVID-19 Deaths') +  
  scale_shape_manual(values=1:nlevels(m5$Region)) +
...
m8 %>%
  ...
  geom_path(size=1) +
  scale_x_continuous(limits=c(-0,90)) + xlab('days since 50th death in region')+ 
  scale_y_log10nice(limits=c(20,1000), name='COVID-19 Deaths per Million ppl') + 
  scale_shape_manual(values=1:nlevels(m5$Region)) +
  theme_bw()
```
...
m8 %>%
  filter(Region=='Spain' | Region=='Germany' | Region=='US' | Region == 'Sweden' | Region == 'Russia' |
