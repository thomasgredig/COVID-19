...
ggplot(m8z, aes(DateDaysNorm, Confirmed/Population*1e6, col=Region)) + 
  ...
  scale_y_log10nice(limits=c(60,5e4), name='Confirmed COVID-19 Cases per Million Population') + 
  scale_x_continuous(limits=c(0,110)) + xlab('Days since 1000th confirmed case') + 
...
```{r}
m8s = subset(m8, Region=='US' | Region=='India' | Region=='Korea, South' | Region =='Sweden' | Region == 'Denmark' | Region=='Switzerland' | Region=='Japan' | Region=='China' | Region=='Italy' | Region=='Brazil')
m8s %>% 
  group_by(Region) %>%
  filter(DateDaysNorm<100) %>%
  summarize(Confirmed = max(Confirmed),
            Population=max(Population), 
            DateDaysNorm = max(DateDaysNorm)) -> m8z
ggplot(m8, aes(DateDaysNorm, Confirmed/Population*1e6, col=Region)) + 
  ...
  scale_y_log10nice(limits=c(10,2e4),name='Confirmed COVID-19 per Million Ppl') + 
  scale_x_continuous(limits=c(0,110)) + xlab('Days since 100th confirmed case') + 
...
m5 %>%
  ...
  geom_point() + 
  scale_x_continuous(limits=c(-30,150)) + xlab('days since Valentine')+ 
...
```{r}
c.list = c('China','Spain','Italy','Denmark','Iran','Japan','Korea, South','Switzerland','US','Brazil', 'Russia')
ggplot(subset(m7, Region %in% c.list), aes(DateDaysNorm, Confirmed, col=Region)) + 
  geom_path(data=m7, col='grey', aes(group=Region)) + 
  geom_point() + 
  scale_x_continuous(limits=c(-0,120)) + xlab('days since 50th death in region')+ 
