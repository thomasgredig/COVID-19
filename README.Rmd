...
Regions with more than 500 confirmed cases increased rapidly from a few to many.
...
```{r}
m8s = subset(m8, Region=='US' | Region=='Spain' | Region=='Korea, South' | Region =='Turkey' |
               Region=='Switzerland' | Region=='Japan' | Region=='China' | Region=='Italy')
m8s %>% 
  group_by(Region) %>%
  filter(DateDaysNorm<45) %>%
  summarize(Confirmed = max(Confirmed),
            Population=max(Population), 
            DateDaysNorm = max(DateDaysNorm)) -> m8z
ggplot(m8, aes(DateDaysNorm, Confirmed/Population*1e6, col=Region)) + 
  ...
  scale_x_continuous(limits=c(0,49)) + xlab('Days since 100th confirmed case') + 
...
Renormalizing the start dates for `recorded deaths`, we find the dates when the `50th death` was recorded in each country:

```{r}
m6= subset(m5, Confirmed >= 50)
...
ggplot(subset(m7, Region %in% c.list), aes(DateDaysNorm, Confirmed, col=Region)) + 
  ...
  scale_x_continuous(limits=c(-0,30)) + xlab('days since 50th death in region')+ 
  scale_y_log10nice(limits=c(50,1e4), name='COVID-19 Deaths') + ylab('Deaths') + 
  scale_shape_manual(values=1:nlevels(m5$Region)) +
  theme_bw()
