...
nls(data = m8a,
    ...
    start = list( T1=3)) -> fit
m8b = data.frame(DateDaysNorm = 0:23,
                 Confirmed = predict(fit, list(DateDaysNorm=0:23)))
...
Growth rates since 100th confirmed case in different countries for first 15 days.

```{r}
result = data.frame()
for (r in levels(droplevels(m8$Region))) {
  m8a = subset(m8, Region == r)
  m8a = subset(m8a, DateDaysNorm <= 15)
  if (nrow(m8a)>5) {
    A0 = m8a$Confirmed[1]
    nls(data = m8a,
      Confirmed ~ A0*exp(DateDaysNorm/T1),
...
ggplot(result, aes(x=reorder(Region,-T), T)) + 
  ...
  scale_y_continuous(limits=c(0,22)) + ylab('confirmed cases doubling time (days)') + 
  coord_flip()
```
