...
m3$population = 0
for(r in m3$Region) {
  if (!(r %in% c("Diamond Princess","Serbia","North Macedonia"))) {
...
m5 %>%
  ...
  ggplot(aes(Date, Confirmed, col=Region)) + 
    geom_line() + 
    scale_y_log10nice(limits=c(100,5e5),name='confirmed COVID-19 cases') + ylab('') + 
...
ggplot(subset(m5, Region=='US' | Region=='France' | Region=='Italy' | Region =='Canada' |
  ...
  scale_y_log10nice(limits=c(100,5e5),name='confirmed COVID-19 cases') + 
  theme_bw()
...
ggplot(m8, aes(DateDaysNorm, Confirmed/Population*1e6, col=Region)) + 
  ...
  scale_x_continuous(limits=c(0,40)) + xlab('Days since 100th confirmed case') + 
  geom_label(data=subset(m8z, Population>7e6),aes(label=Region),hjust=-0.2) + 
  theme_bw() + theme(legend.position = 'none')
```

```{r}
m8s = subset(m8, Region=='US' | Region=='India' | Region=='Korea, South' | Region =='Turkey' |
...
A0 = m8a$Confirmed[1]
nls(data = subset(m8a, DateDaysNorm>10),
    Confirmed ~ A*exp(DateDaysNorm/T1),
    start = list( A=m8a$Confirmed[10], T1=4)) -> fit
m8b = data.frame(DateDaysNorm = 0:35,
                 Confirmed = predict(fit, list(DateDaysNorm=0:35)))
...
ggplot(m8a, aes(DateDaysNorm, Confirmed)) + 
  ...
  scale_x_continuous(limits=c(0,35), breaks=0:4*7, minor_breaks=0:30) +
  theme_bw()
summary(fit)
```
...
ggplot(m8a, aes(DateDaysNorm, Confirmed)) + 
  ...
  ggtitle(paste('US since',min(m8a$Date),' doubling time:',T1,'days')) + xlab('days') +
  scale_y_log10nice(name='Confirmed COVID-19 cases') + 
  scale_x_continuous(limits=c(0,40), breaks=0:5*9, minor_breaks=0:40) +
