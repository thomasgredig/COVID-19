...
ggplot(m8, aes(DateDaysNorm, Confirmed/Population*1e6, col=Region)) + 
  ...
  scale_x_continuous(limits=c(0,30)) + xlab('Days since 100th confirmed case') + 
  geom_label(data=subset(m8z, Population>7e6),aes(label=Region),hjust=-0.2) + 
  theme_bw() + theme(legend.position = 'none')
```

```{r}
m8s = subset(m8, Region=='US' | Region=='France' | Region=='Korea, South' |
...
ggplot(m8a, aes(DateDaysNorm, Confirmed)) + 
  ...
  ggtitle(paste('US since',min(m8a$Date),' doubling time:',T1,'days')) + xlab('days') +
  scale_y_log10nice(name='Confirmed COVID-19 cases') + 
  scale_x_continuous(limits=c(0,28), breaks=0:5*7, minor_breaks=0:30) +
