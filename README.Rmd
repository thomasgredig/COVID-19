...
ggplot(m8z, aes(DateDaysNorm, Confirmed/Population*1e6, col=Region)) + 
  ...
  scale_x_continuous(limits=c(0,70)) + xlab('Days since 1000th confirmed case') + 
...
A0 = m8a$Confirmed[1]
nls(data = subset(m8a, DateDaysNorm>45),
    Confirmed ~ A*exp(DateDaysNorm/T1),
    start = list( A=m8a$Confirmed[10], T1=4)) -> fit
m8b = data.frame(DateDaysNorm = 30:70,
                 Confirmed = predict(fit, list(DateDaysNorm=30:70)))
