...
getPopulation <- function(countryName) {
  ...
  if (length(mc)>= 1) { p = dc$Popoulation.2020[mc] } else { p=1 }
  p/1e6
}
```
...
m = rbind(m1, m4)
m %>%
  filter(Region == 'US' | Region=='Korea, South' | Region=='Arizona' | Region == 'Germany' |
           Region == 'Switzerland' | Region =='Sweden') %>%
  ...
  ggtitle('Confirmed new COVID-19 Cases per Million', subtitle = 
            paste('Source: Johns Hopkins University, ',AVG.DAYS,'day average')) + 
...
AVG.DAYS=5
m %>%
  filter(Region == 'US' | Region=='New York' | Region=='Arizona' | Region == 'California' |
